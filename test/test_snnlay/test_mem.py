...
from VaeInception1d import tdLinear,LIFSpike,tdBatchNorm,MembraneOutputLayer
...
def main():
    ...
    for epoch in tqdm(range(epochs)):
        ...
        for i,data in enumerate(train_inception_loader):
            ...
            out_mem=inception(z).to(float) #[batch x class]
            out_mem=torch.squeeze(out_mem,dim=(0,1))
            print(out_mem)
            
            print("label shape : ",label.shape, type(label),type(label[0].item()))
            print("mem shape : ",out_mem.shape, type(out_mem))
            loss=criterion(out_mem,label.to(int).to(DEVICE))
            # print(label)
        ...
        if (epoch)%conf["save_interval"]==0:
            ...
            with torch.no_grad():
                ...
                for i, data in enumerate(test_loader, 0):
                    ...
                    # print(z.shape,(n,l*t,snn_steps))
                    z=z.reshape(n,l*t,snn_steps)
                    out_mem=inception(z).to(float)
                    y=torch.squeeze(out_mem,dim=(0,1))
